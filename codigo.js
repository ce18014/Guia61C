/*Ada Stephy Cabezas Escobar-CE18014*/
...
function obtenerProductos() {
	 fetch('https://retoolapi.dev/yRfpPg/productos')
		.then(res => res.json())
		.then(data => {
			productos = data;
			productos.forEach(
...
var idi;
function eliminarProductos(idi) {
	
	fetch('https://retoolapi.dev/yRfpPg/productos/'+idi, { method: "DELETE" })
		.then(response => response.json())
		.then(data =>productos = data);
		obtenerProductos();
		
		alert("Se ha eliminado el producto "+idi); 
}
